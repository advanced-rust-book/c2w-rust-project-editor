...
stamp_file="${stamp_dir}/cargo-cache.stamp"
...
if [ -f "${stamp_file}" ]; then
    if [ -z "${cache_key}" ] || grep -qxF "${cache_key}" "${stamp_file}"; then
        echo "hydrate-rust-cache: Cargo cache already hydrated"
        exit 0
    fi
fi

tmp_dir="$(mktemp -d /tmp/c2w-rust-cache.XXXXXX)"
archive_path="${tmp_dir}/cargo-cache.tar.gz"
...
echo "hydrate-rust-cache: unpacking Cargo cache"
